...
const updatePasarScripting = async (lasterVersion, preVersion) => {
  try {
    ...
    console.log(`update NFT Marketplace scripting error: ${error}`);
    throw error;
  }
};
...
const queryPasarScriptingFromDB = async () => {
  try {
    ...
    console.log(`query NFT Marketplace scripting error: ${error}`);
    throw error;
  }
};
...
export const creatAndRegister = async (isForce) => {
  ...
  if (localVersion !== PASAR_SCRIPT_VERSION) {
    try {
      ...
      if (error.code === 404) {
        // ignore 404
        console.log('ignore query NFT Marketplace info 404 error');
      } else {
        console.log('query NFT Marketplace info error: ', error);
        throw error;
      }
    }
  } else {
  ...
  if (PASAR_SCRIPT_VERSION !== lasterVersion) {
    try {
      ...
      console.log('update NFT Marketplace info error: ', error);
    }
